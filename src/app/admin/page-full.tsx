...
function TriggerEditModal({
  ...
  const [form, setForm] = useState({
    ...
    group_exclusions: trigger?.group_exclusions?.join(', ') || '',
    contract_prefix_exclusions: trigger?.contract_prefix_exclusions?.join(', ') || '',
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      group_exclusions: form.group_exclusions ? form.group_exclusions.split(',').map(s => s.trim()).filter(Boolean) : null,
      contract_prefix_exclusions: form.contract_prefix_exclusions ? form.contract_prefix_exclusions.split(',').map(s => s.trim()).filter(Boolean) : null,
    });
  };
        ...
        <div className="p-6">
          ...
          {activeTab === 'details' ? (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">
                  Group Exclusions (comma-separated)
                </label>
                <input
                  type="text"
                  value={form.group_exclusions}
                  onChange={(e) => setForm({ ...form, group_exclusions: e.target.value })}
                  className="w-full px-3 py-2 bg-[#0a1628] border border-[#1e3a5f] rounded-lg text-white text-sm focus:outline-none focus:border-teal-500"
                  placeholder="COS, PDPIND"
                />
                <p className="text-xs text-slate-500 mt-1">
                  Skip opportunities for patients with these group numbers (e.g., COS = Cost share, PDPIND = PDP Individual).
                </p>
              </div>

              <div>
                <label className="block text-xs text-slate-400 mb-1">
                  Contract Prefix Exclusions (comma-separated)
                </label>
                <input
                  type="text"
                  value={form.contract_prefix_exclusions}
                  onChange={(e) => setForm({ ...form, contract_prefix_exclusions: e.target.value })}
                  className="w-full px-3 py-2 bg-[#0a1628] border border-[#1e3a5f] rounded-lg text-white text-sm focus:outline-none focus:border-teal-500"
                  placeholder="S"
                />
                <p className="text-xs text-slate-500 mt-1">
                  Skip if contract ID starts with these prefixes (e.g., S = Part D Standalone, H = Medicare Advantage OK).
                </p>
              </div>
